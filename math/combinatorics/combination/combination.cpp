...
// Combination counting with modulo.
ll combination(ll n, ll r)
{
    if (r > n)
    {
        return 0; // if r > n, there is no combination
    }

    ll res = factorials[n] % mod;
    if (mod == ll(1e9) + 7)
    {
        res = (res * modular_inverse_fast(factorials[r]) % mod) % mod;
        res = (res * modular_inverse_fast(factorials[n - r]) % mod) % mod;
    }
    else
    {
        res = (res * (modular_inverse(factorials[r]) % mod)) % mod;
        res = (res * (modular_inverse(factorials[n - r]) % mod)) % mod;
    }
